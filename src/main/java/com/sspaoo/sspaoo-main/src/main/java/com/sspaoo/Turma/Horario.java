...
public class Horario {
    ...
    public boolean verSeTemAulaNoDia(int dia){
        if(dia > 5 || dia < 1){
            throw new IllegalArgumentException("Os dias da semana vão de 1 a 5");
        }
        return temAulaNesseDia[dia];
    }
    public boolean[] getTemAulaNoDia(){
        return temAulaNesseDia;
    }
    public LocalTime verInicioDaAulaNoDia(int dia){
        if(dia > 5 || dia < 1){
            throw new IllegalArgumentException("Os dias da semana vão de 1 a 5");
        }
    ...
    public LocalTime verFimDaAulaNoDia(int dia){
        if(dia > 5 || dia < 1){
            throw new IllegalArgumentException("Os dias da semana vão de 1 a 5");
        }
